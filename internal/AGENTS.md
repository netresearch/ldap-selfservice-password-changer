...
- `branding.go` derives the UI branding. `Branding.Normalized()` fills only
  _empty_ presentation fields, so an `Opts` built by hand (tests) still renders
  a titled page; it never overrides a configured value and never flips
  `ShowAttribution`, so a deliberate logo-only rebrand survives it.

### web/static/

- `overlay.go` layers `BRANDING_DIR` over the embedded assets. Only the
  allowlist in that file is looked up in the overlay directory — a traversal
  attempt or an unrelated name can never escape into the operator's filesystem,
  and `styles.css` / `js/` are excluded on purpose so a deployment cannot
  silently break the accessibility guarantees the templates depend on.
- Unknown names in the directory are a **startup error**, not a silent no-op,
  so a typo surfaces instead of appearing to work.
- `ValidateOverlay` runs from `options`; `NewOverlay` assumes it already
  passed. `logo-dark.webp` is the only allowlisted asset with no embedded
  counterpart — templates reference it only when `Branding.HasDarkLogo`.
