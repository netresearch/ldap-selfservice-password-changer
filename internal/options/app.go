...
func Parse() *Opts {
	...
	var (
		...
		fPasswordResetEnabled        = flag.Bool("password-reset-enabled", envBoolOrDefault("PASSWORD_RESET_ENABLED", false), "Enable password reset feature.")
		fResetTokenExpiryMinutes     = flag.Uint("reset-token-expiry-minutes", envIntOrDefault("RESET_TOKEN_EXPIRY_MINUTES", 15), "Token validity duration in minutes.")
		fResetRateLimitRequests      = flag.Uint("reset-rate-limit-requests", envIntOrDefault("RESET_RATE_LIMIT_REQUESTS", 3), "Max password reset requests per time window.")
		fResetRateLimitWindowMinutes = flag.Uint("reset-rate-limit-window-minutes", envIntOrDefault("RESET_RATE_LIMIT_WINDOW_MINUTES", 60), "Rate limit time window in minutes.")
		fSMTPHost                    = flag.String("smtp-host", envStringOrDefault("SMTP_HOST", "smtp.gmail.com"), "SMTP server hostname.")
		fSMTPPort                    = flag.Uint("smtp-port", envIntOrDefault("SMTP_PORT", 587), "SMTP server port.")
		fSMTPUsername                = flag.String("smtp-username", envStringOrDefault("SMTP_USERNAME", ""), "SMTP authentication username.")
		fSMTPPassword                = flag.String("smtp-password", envStringOrDefault("SMTP_PASSWORD", ""), "SMTP authentication password.")
		fSMTPFromAddress             = flag.String("smtp-from-address", envStringOrDefault("SMTP_FROM_ADDRESS", ""), "Email sender address.")
		fAppBaseURL                  = flag.String("app-base-url", envStringOrDefault("APP_BASE_URL", ""), "Base URL for password reset links (e.g., https://pwd.example.com).")

		// Optional dedicated service account for password reset (recommended for security)
		fResetUser     = flag.String("reset-user", envStringOrDefault("LDAP_RESET_USER", ""), "Optional dedicated user for password reset operations. Falls back to readonly-user if not set.")
		fResetPassword = flag.String("reset-password", envStringOrDefault("LDAP_RESET_PASSWORD", ""), "Password for the dedicated reset user.")
	)
