...
A rejected request answers `429 Too Many Requests` — except `request-password-reset`, which answers exactly like a served request, because a distinguishable answer would let a caller probe for accounts. The checks run in a fixed order per method (`internal/rpchandler/guards.go`): a malformed or impossible request is refused first, so that rejecting it costs no limiter slot; then the in-memory limiters; and Turnstile verification last, so a flood is rejected without producing outbound requests to Cloudflare.
