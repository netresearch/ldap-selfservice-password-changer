...
A rejected request answers `429 Too Many Requests` — except `request-password-reset`, which answers exactly like a served request, because a distinguishable answer would let a caller probe for accounts. The checks run in a fixed order per method (`internal/rpchandler/guards.go`), with Turnstile verification last, so a flood is rejected without producing outbound requests to Cloudflare. For `change-password` and `request-password-reset` a malformed or impossible request is refused ahead of the limiters, so rejecting one costs no limiter slot; `reset-password` checks its parameters inside the method, after the limiter, as it did before.
